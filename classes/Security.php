...
class Security {
    ...
    public static function checkForgotPasswordAttempts($key, $limit = 5, $minutes = 10)
{
    ...
    foreach ($_SESSION['forgot_password'] as $k => $data) {
        if ($data['expires'] < $now) {
            unset($_SESSION['forgot_password'][$k]);
        }
    }

    if (!isset($_SESSION['forgot_password'][$key])) {
        $_SESSION['forgot_password'][$key] = [
            'count' => 1,
    ...
    if ($_SESSION['forgot_password'][$key]['count'] >= $limit) {
        return false;
    }

    $_SESSION['forgot_password'][$key]['count']++;
    return true;
}
